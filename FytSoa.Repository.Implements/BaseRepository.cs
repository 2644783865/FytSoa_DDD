...
{
    public class BaseRepository<T>:MyContext, IBaseRepository<T> where T : class, new()
    {
        ...
        /// 分页查询条件
        /// </summary>
        /// <param name="where">拉姆达条件表达式</param>
        /// <param name="order">拉姆达排序表达式</param>
        /// <param name="orderEnum">枚举，1=desc 2=asc</param>
        /// <param name="pageIndex">当前第几页</param>
        /// <param name="pageSize">每页总数</param>
        /// <param name="Async">是否同步</param>
        /// <returns></returns>
        public async Task<Page<T>> GetPagesAsync(Expression<Func<T, bool>> where,
            Expression<Func<T, object>> order, int orderEnum, int pageIndex, int pageSize, bool Async = true)
        {
            var query = Db.Queryable<T>()
                        .Where(where)
                        .OrderByIF(orderEnum == 1, order, OrderByType.Asc)
                        .OrderByIF(orderEnum == 2, order, OrderByType.Desc);
            return Async ? await query.ToPageAsync(pageIndex, pageSize) : query.ToPage(pageIndex, pageSize);
        }
