	...
	<section>
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button type="success" icon="el-icon-plus" @click="$refs.group.handleAddGroup()">添加角色组
					</el-button>
				</el-form-item>
				<el-form-item>
		...
		<template>
			<el-table :data="tableData.items" style="width: 100%;margin-bottom: 20px;" row-key="id" v-loading="loading"
				border :height="tableHeight" default-expand-all>
				<el-table-column type="index" width="80" label="序号"></el-table-column>
				<el-table-column prop="name" label="角色名称">
					<template slot-scope="scope">
						<i v-if="scope.row.parentId!='0'" class="el-icon-minus"></i>
						{{scope.row.name}}
						<el-link v-if="scope.row.parentId=='0'" :underline="false" class="cur-tag bg-cyan"
							type="primary">组</el-link>
					</template>
				</el-table-column>
				<el-table-column prop="sort" label="排序" width="180"></el-table-column>
				<el-table-column prop="isSystem" label="是否超管" width="180">
					<template slot-scope="scope">
						<el-link :underline="false" :type="scope.row.isSystem ? 'primary' : 'warning'">
				...
				<el-table-column fixed="right" label="操作" width="160">
					<template slot-scope="scope">
						<el-link icon="el-icon-edit" :underline="false" type="primary"
							...
							@click="$refs.modify.handelModify(scope.row)">修改</el-link>
						<el-link icon="el-icon-edit" :underline="false" type="primary"
							v-if="scope.row.parentId=='0' && !scope.row.isSystem"
							@click="$refs.group.handelModifyGroup(scope.row)">修改</el-link>
						<el-link icon="el-icon-delete" :underline="false" type="danger" @click="deletes(scope.row)"
							v-if="!scope.row.isSystem" style="margin-left:15px;">删除</el-link>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
				:current-page="tableData.currentPage" :page-sizes="[10, 20, 50, 100,200,500,1000]" :page-size="10"
				layout="total, sizes, prev, pager, next, jumper" :total="tableData.totalItems"></el-pagination>
		</template>
